...
%% import data
filename = ['Category learning_Study 1_cleaned.xlsx'];
...
trialBin = repmat(binAssignments,4*length(subjectIDlist),1);
trialCount = repmat(transpose(1:1:40),4*length(subjectIDlist),1);
...
%writetable(data_Table,'Study1.xlsx');
...
% last trial
mTrial160 = mean(rawData.data(rawData.data(:,3)==160,9));
sdTrial160 = std(rawData.data(rawData.data(:,3)==160,9));
